...
import training
from dataset_utils import clean_and_annotate, parse_dataset
...
def _dataset_file(dataset_id):
    files = [
        name for name in os.listdir(training.DATASET_DIR)
        if name.startswith(dataset_id + "_") and "_processed_v" not in name
    ]
    return os.path.join(training.DATASET_DIR, files[0]) if files else None


@app.post("/api/datasets/{dataset_id}/process")
async def process_dataset(dataset_id: str, req: DatasetProcessRequest):
    source_path = _dataset_file(dataset_id)
    if not source_path:
        return {"error": "原始数据集不存在，请先上传文件"}
    if req.minLength < 0 or req.maxLength <= req.minLength:
        return {"error": "长度过滤参数无效：maxLength 必须大于 minLength"}
    try:
        source = parse_dataset(source_path)
        processed, stats = clean_and_annotate(source, req.domain, req.minLength, req.maxLength)
        version = 1
        prefix = f"{dataset_id}_processed_v"
        existing = [name for name in os.listdir(training.DATASET_DIR) if name.startswith(prefix) and name.endswith(".jsonl")]
        if existing:
            version = max(int(name[len(prefix):-6]) for name in existing) + 1
        output_name = f"{dataset_id}_processed_v{version}.jsonl"
        output_path = os.path.join(training.DATASET_DIR, output_name)
        ...
        metadata = {
            ...
            "stats": stats,
            "steps": [
                {"label": "格式检查", "in": stats["input"], "out": stats["input"]},
                {"label": "数据清洗", "in": stats["input"], "out": stats["input"] - stats["blank"]},
                {"label": "去重", "in": stats["input"] - stats["blank"], "out": stats["input"] - stats["blank"] - stats["duplicate"]},
                {"label": "质量过滤", "in": stats["input"] - stats["blank"] - stats["duplicate"], "out": stats["output"]},
                {"label": "标注格式化", "in": stats["output"], "out": stats["output"]},
            ],
            "domain": req.domain or "general",
            "format": "instruction-input-output",
        }
        with open(output_path + ".meta.json", "w", encoding="utf-8") as output:
...
def list_processed_datasets(domain: str = ""):
    datasets = []
    for name in os.listdir(training.DATASET_DIR):
        if not name.endswith(".jsonl") or "_processed_v" not in name:
            continue
        meta_path = os.path.join(training.DATASET_DIR, name + ".meta.json")
        ...
        steps = [
            {**step, "fr": step.get("fr", ["FR-04", "FR-05", "FR-05", "FR-07", "FR-06"][index])}
            for index, step in enumerate(metadata.get("steps", []))
        ]
        display_name = metadata.get("name")
        if not display_name:
            source_prefix = f"{metadata['datasetId']}_"
            source_files = [
                item for item in os.listdir(training.DATASET_DIR)
                if item.startswith(source_prefix) and "_processed_v" not in item and not item.endswith(".meta.json")
            ]
            display_name = source_files[0] if source_files else metadata["filename"]
        datasets.append({
            "id": metadata["datasetId"],
            "name": display_name,
            "version": metadata["version"],
            "rows": stats["output"],
            "updated": metadata["createdAt"][:10],
            "quality": stats["quality"],
            "splits": {"train": 80, "val": 10, "test": 10},
            "steps": steps,
        })
    return {"list": sorted(datasets, key=lambda item: (item["id"], item["version"]), reverse=True)}
    ...
    for name in os.listdir(training.DATASET_DIR):
        if name.startswith(prefix) and name.endswith(".jsonl"):
            ...
            if os.path.exists(meta_path):
                with open(meta_path, "r", encoding="utf-8") as meta_file:
                    versions.append(json.load(meta_file))
    return {"list": sorted(versions, key=lambda item: item["version"], reverse=True)}


@app.get("/api/datasets/{dataset_id}/download/{version}")
def download_dataset(dataset_id: str, version: str):
    filename = f"{dataset_id}_processed_{version}.jsonl"
    path = os.path.join(training.DATASET_DIR, filename)
    if not os.path.isfile(path):
        return {"error": "处理后的数据集版本不存在"}
    return FileResponse(path, filename=filename, media_type="application/jsonl")
