...
SPARSE_PATHS="${SPARSE_PATHS:-requirements.txt run.sh start_app.sh server frontend tests}"  # 云端运行所需顶层路径白名单（稀疏检出）；server/ 含后端入口+config+data_pipeline+model_manager 等，frontend/ 新前端；tests/ 供云端跑数据下载+测试流水线（./deploy.sh datatest），docs/README/CLAUDE/LICENSE/*.bat 等不检出
