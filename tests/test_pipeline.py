...
输出行恒为 {instruction,input,output,source} 且 input==""（单轮）。
"""
...
class _Base(PipelineTestCase):
    ...
    def _assert_rows(self, summary, output_start=None, output_end=None, output_in=None):
        ...
        for r in rows:
            ...
            self.assertTrue(r["instruction"].strip())
...
class TestMedical(_Base):
    ...
    def test_cmb_exam_medical_qa_kept(self):
        self._assert_rows(self._run("cmb_exam_medical.jsonl", "medical_qa"), output_start="答案")

    def test_cmb_clin_medical_qa_kept(self):
        # 真实 CMB-Clin（QA_pairs 用 answer 键）在宽松预设下保留
        self._assert_rows(self._run("cmb_clin_medical.jsonl", "medical_qa"), output_in="诊断")

    def test_toyhom_medical_qa_kept(self):
        self._assert_rows(self._run("toyhom_medical.csv", "medical_qa"), output_in="党参")

    def test_medqa_medical_qa_kept(self):
        self._assert_rows(self._run("medqa_medical.jsonl", "medical_qa"), output_start="答案")

    def test_huatuo_medical_qa_kept(self):
        self._assert_rows(self._run("huatuo_medical.jsonl", "medical_qa"), output_in="党参")

    def test_raw_medical_medical_cn_kept(self):
        # 纯文本生成：含主诉+诊断 → 医疗严格预设保留
        s = self._run("raw_medical.txt", "medical_cn")
        self.assertEqual(s.total, 1); self.assertEqual(s.dropped, 0)
        self._assert_rows(s, output_in="主诉")
...
class TestLegal(_Base):
    ...
    def test_lawbench_qa_legal_qa_kept(self):
        self._assert_rows(self._run("lawbench_qa_legal.jsonl", "legal_qa"), output_start="上述证据")
    ...
    def test_lawbench_summary_legal_qa_kept(self):
        self._assert_rows(self._run("lawbench_summary_legal.jsonl", "legal_qa"), output_in="盗窃罪")

    def test_raw_legal_judgment_legal_cn_kept(self):
        # 真实判决书较长（>默认 max_len），需放大 max_len 才能保留
        ...
        self._assert_rows(s, output_in="判决书")
...
class TestFinance(_Base):
    ...
    def test_fineval_mcq_finance_qa_kept(self):
        self._assert_rows(self._run("fineval_mcq_finance.jsonl", "finance_qa"), output_start="答案")

    def test_fineval_qa_finance_qa_kept(self):
        self._assert_rows(self._run("fineval_qa_finance.jsonl", "finance_qa"))

    def test_fingpt_finance_qa_kept(self):
        self._assert_rows(self._run("fingpt_finance.jsonl", "finance_qa"), output_start="positive")
    ...
    def test_finance_report_cn_kept(self):
        ...
        self.assertEqual(s.total, 1); self.assertEqual(s.dropped, 0); self.assertEqual(s.kept, 1)
        self._assert_rows(s, output_in="亿元")
...
class TestEducation(_Base):
    ...
    def test_mmlu_cn_kept(self):
        ...
        self.assertEqual(s.total, 1); self.assertEqual(s.dropped, 0); self.assertEqual(s.kept, 1)
        self._assert_rows(s, output_start="答案", output_end="D")
    ...
    def test_educhat_education_qa_kept(self):
        self._assert_rows(self._run("educhat_education.jsonl", "education_qa"), output_in="光合作用")
